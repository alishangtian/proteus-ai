    ...
    try:
        ...
        for existing_role in TeamRole:
            if existing_role.value == role_value:
                logger.info(
                    f"角色值 {role_value} 已存在，使用现有角色: {existing_role.name}"
                )
...
class TeamRunnerNode(BaseNode):
    ...
    async def execute(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 导入所需的prompt模板
            from src.agent.prompt.deep_research.coordinator import (
                COORDINATOR_PROMPT_TEMPLATES,
            )
            from src.agent.prompt.deep_research.planner import PLANNER_PROMPT_TEMPLATES
            from src.agent.prompt.deep_research.researcher import (
                RESEARCHER_PROMPT_TEMPLATES,
            )
            from src.agent.prompt.deep_research.coder import CODER_PROMPT_TEMPLATES
            from src.agent.prompt.deep_research.reporter import (
            ...
            logger.error(
                f"[{chat_id}] {error_msg}, 耗时: {execution_time:.2f} 秒", exc_info=True
            )
            ...
            if self.team is not None:
                try:
                    ...
                    logger.error(
                        f"[{chat_id}] 停止团队失败: {str(stop_error)}", exc_info=True
                    )
