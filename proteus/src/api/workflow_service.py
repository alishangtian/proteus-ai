...
class WorkflowService:
    ...
    async def generate_workflow(self, text: str, request_id: str = None) -> Dict:
        ...
        system_prompt = Template(WORKFLOW_GENERATE_SYSTEM_PROMPT).safe_substitute(
            system_values
        )

        user_values = {"nodes_json_example": nodes_json_example}
        user_prompt = Template(WORKFLOW_GENERATE_USER_PROMPT).safe_substitute(
            user_values
        )
    ...
    async def explain_workflow_result(
        ...
        for node in workflow["nodes"]:
            ...
            if node_result and node_result.success:
                ...
                workflow_desc.append(
                    f"- {node_type}({node_id}): 成功，输出={result_data}"
                )
            else:
        ...
        # 使用Template进行格式化填充
        system_values = {"workflow_status": workflow_status}
        system_prompt = Template(WORKFLOW_EXPLAIN_SYSTEM_PROMPT).safe_substitute(
            system_values
        )
