#!/usr/bin/env python3
"""
多任务深度研究子任务启动脚本 - 强制API下发版
所有子任务必须通过API下发执行，不支持本地执行模式
包含增强的错误处理和诊断功能
"""
import json
import time
...
def print_help():
    ...
    print("  --auth-token, -t   指定task下发的鉴权token")
    print("\n示例:")
    ...
    def analyze_network_error(error):
        ...
        if "Name or service not known" in error_msg or "getaddrinfo failed" in error_msg:
            ...
            return "连接超时", "网络延迟过高或服务器响应慢，请检查网络"
                        ...
                        # 容器HTTPS环境可能使用自签名证书
                        # verify=False 允许使用自签名证书，生产环境应配置正确的证书
...
class EnvironmentChecker:
    ...
    def check_local_api_service(endpoint):
        """检查本地API服务"""
        if "nginx" in endpoint or "localhost" in endpoint:
            try:
...
def start_subtask(task_dir, subtask, api_config, max_retries=3, initial_delay=2):
    ...
    # Prepare headers
    endpoint = api_config.get("endpoint", "https://nginx/task")
    auth_token = api_config.get("auth_token", "")  # 必须由用户提供，不能硬编码
...
def start_subtasks_sequential(task_dir, wait_for_completion=False, check_interval=30, max_checks=120, auth_token=None):
...
def start_subtasks_with_dependencies(task_dir, specific_tasks=None, auth_token=None):
    """启动子任务（兼容旧接口）"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        while i < len(sys.argv):
            ...
            if arg == "--wait":
                ...
                while i < len(sys.argv) and not sys.argv[i].startswith("--"):
                    specific_tasks.append(sys.argv[i])
                    i += 1
                continue
            elif arg in ["--auth-token", "-t"]:
                if i + 1 < len(sys.argv):
                    auth_token = sys.argv[i + 1]
                    i += 1  # 跳过token值
                else:
                    print(f"错误: {arg} 需要参数值")
                    sys.exit(1)
            elif arg in ["--help", "-h"]:
                print_help()
                sys.exit(0)
        ...
        if not auth_token:
            print("错误: 鉴权token是必需的。请通过--auth-token参数、TASK_AUTH_TOKEN环境变量提供，或交互式输入。")
            sys.exit(1)
        
        print(f"鉴权token: {auth_token[:4]}...{auth_token[-4:] if len(auth_token) > 8 else '***'}")
        
        # 根据模式调用相应函数
        if mode == "wait":
            print("⏳ 使用顺序等待模式: 每个任务完成后才启动下一个")
            success = start_subtasks_sequential(task_dir, wait_for_completion=True, auth_token=auth_token)
        elif mode == "sequential":
            print("⚡ 使用顺序启动模式: 按依赖顺序启动，但不等待完成")
            success = start_subtasks_sequential(task_dir, wait_for_completion=False, auth_token=auth_token)
        elif mode == "specific":
            print(f"🎯 启动特定任务: {', '.join(specific_tasks)}")
            print("⚠ 注意: 依赖关系可能不被完全遵守")
            success = start_subtasks_with_dependencies(task_dir, specific_tasks, auth_token=auth_token)
        else:
            print(f"⚠ 未知模式: {mode}")
            print("🔧 使用默认顺序启动模式")
            success = start_subtasks_sequential(task_dir, wait_for_completion=False, auth_token=auth_token)
