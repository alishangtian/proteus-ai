...
def init_multi_task_research(task_name, task_description, subtasks=None, base_dir="/app/data/tasks", 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="初始化多任务深度研究项目")
    parser.add_argument("task_name", help="任务名称 (显示名称，可以是中文)")
    parser.add_argument("--folder", "-f", help="任务文件夹名称 (英文，如未提供则基于任务名称生成)")
    parser.add_argument("--description", "-d", help="任务描述 (如未提供则交互式输入)")
    parser.add_argument("--auto-start", "-a", action="store_true", help="自动启动子任务")
    parser.add_argument("--config", "-c", help="JSON配置文件路径 (包含任务和子任务配置)")
    ...
    if args.config and os.path.exists(args.config):
        try:
            ...
            if "task_description" in config_data and not task_description:
                task_description = config_data["task_description"]
            
            if "subtasks" in config_data:
                subtasks = config_data["subtasks"]
                print(f"从配置文件加载 {len(subtasks)} 个子任务")
        except Exception as e:
            print(f"读取配置文件失败: {e}")
            print("将使用交互式输入")
    ...
    if not subtasks:
        ...
        while True:
            ...
            description = input("子任务描述: ").strip()
            
            # 询问英文文件夹名称
            folder_name = input("子任务文件夹名称 (英文，可选，回车自动生成): ").strip()
            if not folder_name:
                folder_name = None
            
            # 询问依赖
            print("输入依赖的子任务名称 (多个用逗号分隔，留空表示无依赖):")
            deps_input = input("依赖: ").strip()
            if deps_input:
                dependencies = [dep.strip() for dep in deps_input.split(",")]
            else:
                dependencies = []
            
            query = input("研究查询语句 (可选，回车使用描述): ").strip()
            if not query:
                query = description
            ...
            if folder_name:
                subtask_config["folder_name"] = folder_name
    ...
    init_multi_task_research(
        ...
        task_description=task_description,
        subtasks=subtasks,
        task_folder_name=args.folder,
        auto_start_subtasks=auto_start
    )
